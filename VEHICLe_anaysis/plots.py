...
def plot_animation_3D(xdata, ydata, zdata, regids, outname):
    ...
    def init():
        ...
        plt.xlabel('\u0394acidities/kcalpermol')
        plt.ylabel('\u0394electrophile affinities/kcalpermol')
        ax1.set_zlabel('\u0394properties/kcalpermol')
        plt.title('Delta Properties 3D')
        plt.xlim(-5, 30)
        plt.ylim(-5, 35)
...
def plot_activation_map(calculate_properties_dataframe, outname):
    ...
    for i in regid:
        ...
        for m, c in acidities.items():
            if c == (min(acidities.values())):
                relative_acidities.append(c / 24.038503) if c/24.038503 not in relative_acidities else relative_acidities
    ...
    plt.gca().add_patch(Rectangle((0, 0), 2, 1, color='salmon'))
    plt.gca().add_patch(Rectangle((1, 0), 1, 5, color='lightblue'))
    plt.gca().add_patch(Rectangle((1, 0), 1, 1, color='mediumpurple'))
    ax1.legend(bbox_to_anchor=(1.05,1,2,1))
    plt.xlim(0.6, 1.25)
    plt.ylim(0, 2.6)
